...
// Implementacion del repositorio para manejar datos de mascotas
class MascotaRepositoryImpl : MascotaRepository {

    // Lista mutable para almacenar mascotas en memoria
    private val _mascotas = MutableStateFlow<List<Mascota>>(emptyList())
    private var nextId = 1

    override fun getAll(): Flow<List<Mascota>> = _mascotas.asStateFlow()

    // Obtiene la lista actual de mascotas sin usar flujos
    ...
    override suspend fun add(mascota: Mascota): Result<Mascota> {
        return try {
            delay(800) // Simulamos retraso de red
    ...
    override suspend fun update(mascota: Mascota): Result<Mascota> {
        return try {
            delay(800) // Simulamos tiempo de espera
