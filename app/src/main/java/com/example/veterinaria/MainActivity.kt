...
import com.example.veterinaria.ui.LoginScreen
import com.example.veterinaria.ui.MascotaFormScreen
import com.example.veterinaria.ui.MascotasScreen
import com.example.veterinaria.ui.RecuperarPasswordScreen
...
// Actividad principal que inicia la aplicacion
class MainActivity : ComponentActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        // Iniciamos el servicio de sincronizacion
        iniciarServicioSincronizacion()

        // Configuramos la interfaz grafica
        setContent {
        ...
        // Verificamos si hay intents externos
        manejarIntentExterno(intent)
    }

    override fun onNewIntent(intent: Intent?) {
        super.onNewIntent(intent)
        // Procesamos nuevos intents si la app ya estaba abierta
        intent?.let { manejarIntentExterno(it) }
    }

    // Metodo para arrancar el servicio en segundo plano
    private fun iniciarServicioSincronizacion() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                // Para versiones nuevas de Android usamos foreground
    ...
    // Gestion de enlaces o datos compartidos desde otras apps
    private fun manejarIntentExterno(intent: Intent?) {
        when (intent?.action) {
            Intent.ACTION_VIEW -> {
                // Aqui podriamos manejar deep links
                val data = intent.data
            }
            Intent.ACTION_SEND -> {
                // Aqui manejamos texto compartido
...
// Configuración de la navegacion entre pantallas
...
fun VeterinariaApp() {
    ...
    // Obtenemos la ruta actual para controlar la visibilidad del topbar
    ...
    Scaffold(
        ...
        NavHost(
            ...
            startDestination = "login", // Iniciamos en el login
            ...
            // Definicion de cada pantalla de la app
            ...
            // Rutas con argumentos para formularios
            composable(
